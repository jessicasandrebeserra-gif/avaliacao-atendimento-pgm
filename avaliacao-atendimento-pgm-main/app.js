...
function renderSubgroupOptions(rating) {
...
function toggleSubgroup(event, label, element) {
...
function checkSubmit() {
  ...
  // O botão só é habilitado quando os quatro campos obrigatórios estão preenchidos:
  // nome válido + atendente selecionado + nota selecionada + pelo menos um subgrupo selecionado
...
function updateDashboard() {
  ...
  data.forEach(d => {
    ...
    if (Array.isArray(d.subgrupos)) {
      ...
      subgroupCounts[d.subgrupo] = (subgroupCounts[d.subgrupo] || 0) + 1;
    }
  });
  ...
  if (subgroupBars) {
    ...
    const total = data.length || 1;
    const sortedSubgroups = Object.entries(subgroupCounts)
      .sort((a, b) => b[1] - a[1]);
